...
# 创建 UCI 默认配置文件
mkdir -p files/etc/uci-defaults
> files/etc/uci-defaults/99-custom  # 创建空文件
...
if [ "$Enable_IPV6_function" == "1" ]; then
    sed -i "s/# CONFIG_PACKAGE_ipv6helper is not set/CONFIG_PACKAGE_ipv6helper=y/" .config
    [ "$Create_Ipv6_Lan" == "1" ] && Create_Ipv6_Lan="0"
fi
...
if [ "$Password_free_login" == "1" ]; then
    cat << EOF >> files/etc/uci-defaults/99-custom
uci set system.@system<source_id data="0" title="diy-part2-798x.sh" />.password=""
uci commit system
EOF
fi
...
if [ "$Ttyd_account_free_login" == "1" ]; then
    cat << EOF >> files/etc/uci-defaults/99-custom
uci set ttyd.@ttyd<source_id data="0" title="diy-part2-798x.sh" />.command='/bin/sh -l'
uci commit ttyd
EOF
fi
...
if [ "$Disable_53_redirection" == "1" ]; then
    cat << EOF >> files/etc/uci-defaults/99-custom
uci del firewall.@redirect<source_id data="0" title="diy-part2-798x.sh" />
uci commit firewall
EOF
fi
...
for file in $(egrep "终端" -rl ./); do
    [ -f "$file" ] && sed -i 's/"终端"/"终端"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "aMule设置" -rl ./); do
    [ -f "$file" ] && sed -i 's/"aMule设置"/"电驴下载"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "网络存储" -rl ./); do
    [ -f "$file" ] && sed -i 's/"网络存储"/"NAS"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "Turbo ACC 网络加速" -rl ./); do
    [ -f "$file" ] && sed -i 's/"Turbo ACC 网络加速"/"网络加速"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "实时流量监测" -rl ./); do
    [ -f "$file" ] && sed -i 's/"实时流量监测"/"流量"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "KMS 服务器" -rl ./); do
    [ -f "$file" ] && sed -i 's/"KMS 服务器"/"KMS激活"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "TTYD 终端" -rl ./); do
    [ -f "$file" ] && sed -i 's/"TTYD 终端"/"TTYD终端"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "USB 打印服务器" -rl ./); do
    [ -f "$file" ] && sed -i 's/"USB 打印服务器"/"打印服务"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "Web 管理" -rl ./); do
    [ -f "$file" ] && sed -i 's/"Web 管理"/"Web管理"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "管理权" -rl ./); do
    [ -f "$file" ] && sed -i 's/"管理权"/"管理权"/g' "$file" || echo "Warning: $file not found"
done
for file in $(egrep "带宽监控" -rl ./); do
    [ -f "$file" ] && sed -i 's/"带宽监控"/"监控"/g' "$file" || echo "Warning: $file not found"
done
