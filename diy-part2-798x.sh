...
# -------  以下为需要写入固件的初始值设定  -------
# 请注意：以下修改将在编译时直接修改固件内的默认配置文件，
#        刷入固件后即为初始值。如需运行时修改，请在OpenWrt后台进行。
...
# -------  以下为在线更新时删除不想保留固件的某个文件  -------
cat >>$DELETE <<-EOF
EOF

# -------  以下为将设定的初始值写入固件  -------
# 注意：以下命令需要在 OpenWrt 编译环境的根目录下执行，且在 make image 之前执行

ROOTFS_DIR="./build_dir/*/root-*" # 假设 rootfs 目录在此路径下，请根据实际情况调整

if [ -n "$Ipv4_ipaddr" ] && [ "$Ipv4_ipaddr" != "0" ]; then
  sed -i "s/option ipaddr '192.168.1.1'/option ipaddr '$Ipv4_ipaddr'/g" "$ROOTFS_DIR/etc/config/network"
fi

if [ -n "$Netmask_netm" ] && [ "$Netmask_netm" != "0" ]; then
  sed -i "s/option netmask '255.255.255.0'/option netmask '$Netmask_netm'/g" "$ROOTFS_DIR/etc/config/network"
fi

if [ -n "$Op_name" ] && [ "$Op_name" != "0" ]; then
  sed -i "s/option hostname 'OpenWrt'/option hostname '$Op_name'/g" "$ROOTFS_DIR/etc/config/system"
fi

if [ -n "$Default_theme" ] && [ "$Default_theme" != "0" ]; then
  sed -i "s/option theme 'bootstrap'/option theme '$Default_theme'/g" "$ROOTFS_DIR/etc/config/luci"
fi

if [ -n "$Gateway_Settings" ] && [ "$Gateway_Settings" != "0" ]; then
  sed -i "/config interface 'lan'/a\ \ \ \ \ \ \ option gateway '$Gateway_Settings'" "$ROOTFS_DIR/etc/config/network"
fi

if [ -n "$DNS_Settings" ] && [ "$DNS_Settings" != "0" ]; then
  sed -i "/config interface 'lan'/a\ \ \ \ \ \ \ option dns '$DNS_Settings'" "$ROOTFS_DIR/etc/config/network"
fi

if [ "$Disable_DHCP" == "1" ]; then
  sed -i "s/option dhcp 'server'/option dhcp 'none'/g" "$ROOTFS_DIR/etc/config/network"
fi

if [ "$Disable_Bridge" == "1" ]; then
  sed -i "s/option bridge_empty '1'/option bridge_empty '0'/g" "$ROOTFS_DIR/etc/config/network" # 如果有 bridge_empty 选项，则修改
  sed -i "s/option ifname 'br-lan'/option ifname 'eth0'/g" "$ROOTFS_DIR/etc/config/network" # 假设网卡是 eth0，请根据实际情况修改
fi

if [ -n "$Broadcast_Ipv4" ] && [ "$Broadcast_Ipv4" != "0" ]; then
  sed -i "/config interface 'lan'/a\ \ \ \ \ \ \ option broadcast '$Broadcast_Ipv4'" "$ROOTFS_DIR/etc/config/network"
fi

if [ "$Customized_Information" != "0" ]; then
  echo "$Customized_Information" > "$ROOTFS_DIR/etc/banner" # 修改 banner，也可以修改 motd 等
fi

if [ "$Password_free_login" == "1" ]; then
  sed -i 's/^root:\$1\$[^\$]*\$[^\$]*:/:x:/' "$ROOTFS_DIR/etc/shadow" # 移除 root 用户的密码哈希，使其无密码
fi

echo "固件初始值设定已完成。"
echo "请确保在 OpenWrt 编译环境根目录下，且在 make image 之前执行此脚本。"